 ...
 */
enum class ResultType(var code: String, var message: String, var httpStatus: HttpStatus, var record: Boolean = false) {
    Success("00000", "", HttpStatus.OK),
    ClientError("A0000", "用户端错误", HttpStatus.BAD_REQUEST, true),
    ...
    LoginFailedExceed("A0213", "用户输入密码次数超限", HttpStatus.BAD_REQUEST, true),
    LoginExpired("A0220", "用户登录已过期", HttpStatus.UNAUTHORIZED),
    TokenInvalid("A0221", "token 无效或已过期", HttpStatus.UNAUTHORIZED),
    ThirdLoginFailed("A0230", "用户第三方登录失败", HttpStatus.UNAUTHORIZED, true),
    ThirdLoginCaptchaError("A0232", "用户第三方登录验证码错误", HttpStatus.UNAUTHORIZED, true),
    ThirdLoginExpired("A0233", "用户第三方登录已过期", HttpStatus.UNAUTHORIZED, true),
    ...
    JSONParseFailed("A0410", "请求 JSON 解析错误", HttpStatus.BAD_REQUEST, true),
    ...
    UploadError("A0600", "用户上传文件异常", HttpStatus.BAD_REQUEST, true),
    ...
    ServerError("B0000", "系统执行出错", HttpStatus.INTERNAL_SERVER_ERROR, true),
    ServerTimeout("B0100", "系统执行超时", HttpStatus.INTERNAL_SERVER_ERROR, true),
    ServerResourceError("B0200", "系统资源异常", HttpStatus.INTERNAL_SERVER_ERROR, true),
    ThirdServiceError("C0000", "调用第三方服务出错", HttpStatus.INTERNAL_SERVER_ERROR, true),
    MiddlewareError("C0100", "中间件服务出错", HttpStatus.INTERNAL_SERVER_ERROR, true),
    ThirdServiceTimeoutError("C0200", "第三方系统执行超时", HttpStatus.INTERNAL_SERVER_ERROR, true),
    DatabaseError("C0300", "数据库服务出错", HttpStatus.INTERNAL_SERVER_ERROR, true),
    CacheError("C0HttpStatus.BAD_REQUEST", "缓存服务出错", HttpStatus.INTERNAL_SERVER_ERROR, true),
    NotificationError("C0500", "通知服务出错", HttpStatus.INTERNAL_SERVER_ERROR, true),
}